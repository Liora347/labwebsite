---
layout: page
title: Research
permalink: /research/
---
<h1 style="font-family: Georgia, serif; font-weight: 600;">Research</h1>

<div class="nature-hero">
  <img src="{{ '/images/meshorer lab 2024.jpg' | relative_url }}"
       style="width:100%; max-width:1000px; display:block; margin:30px auto; border-radius:10px;">
</div>

<h2 style="font-family: Georgia, serif;">BindDB</h2>

<p><strong>BindDB: The Meshorer Lab Epigenomic Database</strong></p>

<p>
  <a href="http://bind-db.huji.ac.il/" class="pub-btn">Open BindDB</a>
</p>

<p>
  <a href="http://meshorerlab.huji.ac.il/papers/LivyatanCellStemCell2015.pdf" class="pub-btn">
    Livyatan et al., Cell Stem Cell (2015)
  </a>
</p>

<hr>

<p>The Meshorer lab is studying:</p>

<ol>
  <li>Epigenetic regulation in stem cells and differentiation.</li>
  <li>Neurodegenerative diseases (e.g. HD) using pluripotent stem cell models.</li>
  <li>Paleo-epigenetics: reconstructing DNA methylation in archaic genomes.</li>
</ol>

<hr>

<h2>Chromatin plasticity in embryonic stem cells</h2>

<img src="{{ '/images/CPlasticity.jpg' | relative_url }}"
     style="width:100%; max-width:900px; display:block; margin:20px auto; border-radius:10px;">

<p>
Using endogenously-labeled fluorescent libraries in mouse embryonic stem cells which we generated, we can visualize differentiation events, nuclear body dynamics, protein binding kinetics, etc. and screen for pluripotency and differentiation relevant factors, in living cells.
</p>

<h2>Human pluripotent stem cells</h2>

<img src="{{ '/images/HPSCs.png' | relative_url }}"
     style="width:100%; max-width:900px; display:block; margin:20px auto; border-radius:10px;">

<p>
Here we are using human embryonic stem cells (hESCs) and human induced pluripotent stem cells (iPSCs) to model neurodegenerative diseases. We are studying poly-glutamine (PolyQ) diseases including Huntington's disease (HD) and Machado Joseph disease (MJD), as well as Parkinson's disease (PD).
</p>

<h2>Chromatin and transcription in ESCs</h2>

<p>
In our ERC project, we aim to understand chromatin plasticity and the function of non-polyadenylated transcription in pluripotency. We are combining biochemistry, single cell advanced imaging assays and high throughput technologies to study chromatin and transcription at a genome-wide scale in ES cells and during differentiation and reprogramming, as well as decipher the function of candidate chromatin proteins and candidate non-polyadenylated transcripts in pluripotency.
</p>

<p>
  <a href="https://erc.europa.eu/about-erc" class="pub-btn">About the ERC</a>
</p>

<h2>Collaborators</h2>

<ul>
  <li>Gil Ast, Tel-Aviv University</li>
  <li>Gustavo Mostoslavsky, Boston University</li>
  <li>Karsten Rippe, DKFZ</li>
  <li>Paola Scaffidi, London Research Institute</li>
  <li>Newman Sze, Nanyang Technological University, Singapore</li>
  <li>Takumi Takizawa, Gunma University, Japan</li>
</ul>
